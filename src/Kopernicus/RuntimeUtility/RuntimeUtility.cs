        ...
        private void WriteConfigIfNoneExists()
        {
            if (!File.Exists(PluginPath + "/../Config/Kopernicus_Config.cfg"))
            {
                ...
                using (StreamWriter configFile = new StreamWriter(PluginPath + "/../Config/Kopernicus_Config.cfg"))
                {
                    ...
                    configFile.WriteLine("	ShadowDistanceLimit = 17500 //Integer.  A number defining the maximum distance at which shadows may be cast.  Lower numbers yield less shadow cascading artifacts, but higher numbers cast shadows farther. Default at 17500 is an approximation of stock. Only works if EnableKopernicusShadowManager is true.");
        ...
        private void UpdateConfig()
        {
            ...
            if (!File.Exists(PluginPath + "/../Config/Kopernicus_Config.cfg"))
            {
                ...
                using (StreamWriter configFile = new StreamWriter(PluginPath + "/../Config/Kopernicus_Config.cfg"))
                {
                    ...
                    configFile.WriteLine("	ShadowDistanceLimit = " + KopernicusConfig.ShadowDistanceLimit + " //Integer.  A number defining the maximum distance at which shadows may be cast.  Lower numbers yield less shadow cascading artifacts, but higher numbers cast shadows farther. Default at 17500 is an approximation of stock. Only works if EnableKopernicusShadowManager is true.");
